...
import { RCode } from 'src/common/constant';
...
export class ChatGateway {
  ...
  // 获取所有群和好友数据
  @SubscribeMessage('ServerChatData')
  async getServerChatData(
    @ConnectedSocket() client: Socket,
    ...
    if (isUser) {
      ...
      this.server.to(user.userId).emit('ServerChatData', {
        code: RCode.OK,
        msg: '获取聊天数据成功',
        data: {
          groupData: groupArr,
  ...
  // 客户端只需要获取私聊消息
  @SubscribeMessage('CustomerChatData')
  async getAllCustomerData(
    ...
    if (isUser) {
      ...
      userArr = await this.userRepository.find();

      this.server.to(user.userId).emit('CustomerChatData', {
        code: RCode.OK,
        msg: '获取聊天数据成功',
        data: {
          friendData: friendArr,
  ...
  // 分配客服
  @SubscribeMessage('AssignServer')
  async assignServer(
    @ConnectedSocket() client: Socket,
    @MessageBody() user: User,
  ): Promise<any> {
    // 找到客服，然后跟客服建立好友，建立socket
    const server = await this.userRepository.findOne({ role: 'server' });

    const roomId =
      user.userId > server.userId
        ? user.userId + server.userId
        : server.userId + user.userId;
    // 存两条数据，一条正向，一条反向
    const friendInfo = {
      userId: user.userId,
      friendId: server.userId,
    };
    const isExit = await this.friendRepository.findOne(friendInfo);
    if (!isExit) {
      await this.friendRepository.save(friendInfo);
    }
    const reverseInfo = {
      userId: server.userId,
      friendId: user.userId,
    };
    const isReverseExit = await this.friendRepository.findOne(reverseInfo);
    if (!isReverseExit) {
      await this.friendRepository.save(reverseInfo);
    }
    client.join(roomId);
    console.log(roomId, '客户进的房间');
    this.server.to(user.userId).emit('AssignServer', {
      code: RCode.OK,
      msg: `分配${server.username}客服成功`,
      data: server,
    });
    // 新分配客户
    if (!isExit || !isReverseExit) {
      this.server.to(server.userId).emit('AssignServer', {
        code: RCode.OK,
        msg: `分配${user.username}客户成功`,
        data: user,
      });
    }
  }
  ...
  // 客户闲置之后重连
  @SubscribeMessage('ReconnectServer')
  async reconnectServer(
    @ConnectedSocket() client: Socket,
    @MessageBody() user: FriendMessageDto,
  ): Promise<any> {
    const roomId =
  ...
  // 加入私聊的socket连接
  @SubscribeMessage('ReceptionCustomer')
  async receptionCustomer(
    @ConnectedSocket() client: Socket,
    @MessageBody() data: UserMap,
  ): Promise<any> {
    if (data.friendId && data.userId) {
      ...
      if (relation) {
        client.join(roomId);
        console.log(roomId, '客服进的房间');
        this.server.to(data.userId).emit('ReceptionCustomer', {
          code: RCode.OK,
          msg: '接待客户成功',
          data: relation,
        });
  ...
  async customerMessage(
    ...
    const roomId =
      data.userId > data.friendId
        ...
        : data.friendId + data.userId;
    if (data.messageType === 'image') {
      console.log(data.content, '图片消息');
      console.log(imageinfo(data.content), '图片消息');
      const { format } = imageinfo(data.content);
      const randomName = `${Date.now()}$${roomId}.${format.toLowerCase()}`;
      const stream = createWriteStream(join('public/static', randomName));
      stream.write(data.content);
      data.content = 'public/static/' + randomName;
    }
    data.time = new Date().valueOf();
    client.join(roomId);
    await this.friendMessageRepository.save(data);
    this.server
      .to(roomId)
      .emit('CustomerMessage', { code: RCode.OK, msg: '', data });
  }
